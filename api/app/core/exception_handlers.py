...
async def storage_error_handler(request: Request, exc: StorageError):
    """Object-storage failures → a clean 503, not a bare 500.

    ``StorageError`` is raised both by the storage operations themselves
    and — critically — by ``get_storage_client`` at dependency-resolution
    time when the Supabase env vars are missing. A route's own
    ``try/except StorageError`` can't catch the dependency-resolution
    raise, so without this handler it escalated to an uncaught 500.

    That mattered for more than the status code: an uncaught 500 is
    generated by Starlette's ServerErrorMiddleware, which sits ABOVE
    CORSMiddleware, so the response carried no ``Access-Control-Allow-
    Origin`` header and browsers surfaced it as a misleading "network
    error." Handling it here routes the response back through CORS and
    gives clients a real, actionable envelope.
    """
    return JSONResponse(
        status_code=503,
        content=_envelope(
